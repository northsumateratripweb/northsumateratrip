...
Route::get('/packages', function () {
    $products = Product::active()->orderBy('sort_order')->paginate(12);
...
Route::prefix('rental-package')->group(function () {
    ...
    Route::get('/{slug}', [RentalPackageController::class, 'show'])->name('rental-package.show');
    Route::post('/{slug}/order', [RentalPackageController::class, 'storeOrder'])->name('rental-package.order')->middleware('throttle:5,1');
});
...
Route::prefix('product')->group(function () {
    ...
    Route::post('/{product:slug}/review', [ReviewController::class, 'store'])->name('products.review')->middleware('throttle:3,1');
    Route::post('/{product:slug}/order', [OrderController::class, 'store'])->name('products.order')->middleware('throttle:5,1');
});
...
// Legal Pages
Route::get('/terms', [StaticPageController::class, 'show'])->defaults('slug', 'terms')->name('legal.terms');
Route::get('/privacy', [StaticPageController::class, 'show'])->defaults('slug', 'privacy')->name('legal.privacy');
...
Route::get('/{page:slug}', [PageController::class, 'show'])->name('pages.static');

// Laporan Pesanan (protected — admin only)
Route::middleware(['auth'])->prefix('laporan')->group(function () {
    Route::get('/dashboard', [LaporanPesananController::class, 'dashboard'])->name('dashboard.laporan');
    Route::get('/pesanan', [LaporanPesananController::class, 'laporan'])->name('laporan.pesanan');
    Route::get('/pesanan/export-csv', [LaporanPesananController::class, 'exportCsv'])->name('laporan.pesanan.csv');
    Route::get('/pesanan/export-excel', [LaporanPesananController::class, 'exportExcel'])->name('laporan.pesanan.excel');
    Route::post('/trip-import', [TripImportController::class, 'import'])->name('trip.import');
});
