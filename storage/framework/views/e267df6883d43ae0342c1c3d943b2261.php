...
<?php $__env->startSection('title', 'Cek Status Pesanan - ' . ($settings['site_name'] ?? 'NorthSumateraTrip')); ?>
    ...
    <div class="container mx-auto px-4 max-w-xl">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('success')): ?>
            <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/40 rounded-2xl p-8 mb-8 text-center">
                ...
                <p class="text-slate-600 dark:text-slate-400 font-medium mb-6"><?php echo e(session('success')); ?></p>
                
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('whatsapp_url')): ?>
                <a href="<?php echo e(session('whatsapp_url')); ?>" target="_blank" class="inline-flex items-center gap-3 px-7 py-3.5 bg-[#25D366] hover:bg-[#1fb355] text-white font-bold rounded-xl transition-all shadow-lg shadow-[#25D366]/20 uppercase tracking-wider text-xs">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8">
            <form method="POST" action="<?php echo e(route('booking.check')); ?>">
                <?php echo csrf_field(); ?>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Order ID</label>
                    <input type="text" name="order_id" value="<?php echo e(request('order_id')); ?>" required class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all">
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Nomor Telepon</label>
                    <input type="text" name="phone" value="<?php echo e(request('phone')); ?>" required class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all" placeholder="08xx...">
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 mt-8" 
             x-data="{ 
                ...
                statusLabel: '<?php echo e($order->status_label ?? ucfirst($order->status)); ?>',
                polling: true,
                async checkStatus() {
                    ...
                    try {
                        let response = await fetch('<?php echo e(route('api.order.status', $order->id)); ?>?phone=<?php echo e($order->customer_phone); ?>');
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between border-b pb-2">
                    ...
                    <span class="font-semibold"><?php echo e($order->product?->name ?? $order->vehicle?->name ?? '-'); ?></span>
                </div> REPLACE
<<<<<<< SEARCH
                ...
                <div class="flex justify-between border-b border-blue-100 pb-2">
                    ...
                    <span class="font-semibold text-slate-800"><?php echo e($order->tripSchedule->vehicle->name); ?> (<?php echo e($order->tripSchedule->vehicle->license_plate ?? '-'); ?>)</span>
                </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
=======
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($order->tripSchedule?->vehicle): ?>
                ...
                <div class="flex justify-between border-b pb-2">
                    ...
                    <span class="font-semibold text-right text-sm max-w-xs"><?php echo e($order->notes); ?></span>
                </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                <div class="flex justify-between border-b pb-2">
                    <span class="text-slate-500">Tanggal Trip</span>
                    <span class="font-semibold"><?php echo e(\Carbon\Carbon::parse($order->trip_date)->format('d M Y')); ?></span>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <span class="text-slate-500">Jumlah</span>
                    <span class="font-semibold"><?php echo e($order->quantity); ?> <?php echo e($order->vehicle_id ? 'hari' : 'orang'); ?></span>
                </div>
                <div class="flex justify-between border-b pb-2">
                    <span class="text-slate-500">Total</span>
                    <span class="font-bold text-green-600"><?php echo e(currency($order->total_price)); ?></span>
                </div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl shadow-lg p-8 mt-6">
            ...
            <div class="space-y-3">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($order->tripSchedule->driver_name): ?>
                <div class="flex justify-between border-b border-blue-100 pb-2">
                    <span class="text-blue-600 font-medium">Driver</span>
                    <span class="font-semibold text-slate-800"><?php echo e($order->tripSchedule->driver_name); ?></span>
                </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div class="flex justify-between border-b border-blue-100 pb-2">
                    ...
                    <a href="https://wa.me/<?php echo e(preg_replace('/\D/', '', $order->tripSchedule->driver_phone)); ?>" target="_blank" class="font-semibold text-green-600 hover:underline">
                        <?php echo e($order->tripSchedule->driver_phone); ?>
                ...
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($order->tripSchedule->trip_date): ?>
                <div class="flex justify-between border-b border-blue-100 pb-2">
                    <span class="text-blue-600 font-medium">Jadwal Keberangkatan</span>
                    <span class="font-semibold text-slate-800"><?php echo e(\Carbon\Carbon::parse($order->tripSchedule->trip_date)->format('d M Y, H:i')); ?></span>
                </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                <div class="flex justify-between">
                    <span class="text-blue-600 font-medium">Status Jadwal</span>
                    <span class="px-3 py-1 rounded-full text-sm font-semibold
                        <?php if($order->tripSchedule->status == 'completed'): ?> bg-green-100 text-green-800
                        <?php elseif($order->tripSchedule->status == 'ongoing'): ?> bg-yellow-100 text-yellow-800
                        <?php elseif($order->tripSchedule->status == 'scheduled'): ?> bg-blue-100 text-blue-800
                        <?php elseif($order->tripSchedule->status == 'cancelled'): ?> bg-red-100 text-red-800
                        <?php else: ?> bg-slate-50 text-slate-800
                        <?php endif; ?>">
                        <?php echo e($order->tripSchedule->status_label ?? ucfirst($order->tripSchedule->status)); ?>
        ...
        <div class="mt-6 text-center">
            <a href="<?php echo e(route('order.invoice', $order->id)); ?>" target="_blank" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors text-xs uppercase tracking-wider shadow-lg shadow-blue-500/20">
