                ...
                @foreach($products as $product)
                    <a href="{{ route('products.show', [$product->category->slug, $product->slug]) }}" class="group flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 transition-all duration-500 hover:shadow-xl hover:shadow-blue-900/[0.06] hover:-translate-y-1 overflow-hidden block cursor-pointer">
                        ...
                        <div class="relative h-52 overflow-hidden">
                            ...
                            <!-- Wishlist Button -->
                            <form action="{{ route('wishlist.toggle', $product->id) }}" method="POST" class="absolute top-4 right-4 z-10" onclick="event.stopPropagation(); event.preventDefault(); this.submit();">
                                @csrf
                                <button type="submit" class="w-10 h-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-xl flex items-center justify-center text-slate-400 hover:text-rose-500 transition-all shadow-sm border border-white/20 dark:border-slate-800 active:scale-90">
                                    <svg class="w-5 h-5 {{ in_array($product->id, $wishlistedProductIds ?? []) ? 'fill-rose-500 text-rose-500' : 'fill-none' }}" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
