    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-16 mb-14">
            ...
            <div class="lg:col-span-5">
                <a href="{{ route('home') }}" class="group flex items-center gap-3 mb-7">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-medium text-slate-500 uppercase tracking-wider leading-none mt-0.5">{{ __('ui.site_premium') }}</p>
                    </div>
                </a>
                <p class="text-slate-400 text-sm leading-relaxed mb-8 max-w-sm">
                    {{ $settings['site_description'] ?? 'Layanan private trip premium untuk pengalaman perjalanan terbaik Anda.' }}
                </p>
                <div class="flex items-center gap-2.5">
                    @foreach(['facebook', 'instagram', 'tiktok', 'youtube'] as $social)
                        @if($settings[$social.'_url'] ?? false)
                            <a href="{{ $settings[$social.'_url'] }}" target="_blank" class="w-9 h-9 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-center text-slate-500 hover:bg-blue-700 hover:text-white hover:border-blue-700 transition-all duration-200">
            ...
            <div class="lg:col-span-3">
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <h4 class="text-xs font-bold text-slate-500 uppercase tracking-widest">{{ __('ui.quick_links') }}</h4>
                </div>
                <ul class="space-y-3.5">
                    @foreach([
                        ...
                        <li>
                            <a href="{{ route($link['route']) }}" class="text-slate-400 hover:text-blue-400 text-sm transition-colors flex items-center gap-2 group">
            ...
            <div class="lg:col-span-4">
                <div class="flex items-center gap-2 mb-5">
                    <div class="w-5 h-0.5 bg-blue-600"></div>
                    <h4 class="text-xs font-bold text-slate-500 uppercase tracking-widest">{{ __('ui.contact_us_footer') }}</h4>
                </div>
                <div class="space-y-5">
        ...
        <div class="pt-8 border-t border-slate-900 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p class="text-slate-600 text-xs">
                &copy; {{ date('Y') }} <span class="text-slate-400 font-medium">NorthSumateraTrip</span>. {{ __('ui.all_rights') }}
            </p>
            <div class="flex items-center gap-6">
                <a href="{{ route('legal.privacy') }}" class="text-slate-600 hover:text-slate-300 text-xs transition-colors">{{ __('ui.privacy_policy') }}</a>
                <a href="{{ route('legal.terms') }}" class="text-slate-600 hover:text-slate-300 text-xs transition-colors">{{ __('ui.terms_of_service') }}</a>
            </div>
        </div>
    </div>
</footer>
